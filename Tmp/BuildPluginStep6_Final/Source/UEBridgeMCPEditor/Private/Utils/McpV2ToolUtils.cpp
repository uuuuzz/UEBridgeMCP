...
namespace
{
	...
	TSharedPtr<FJsonObject> MakeBoundsJson(const FVector& Origin, const FVector& BoxExtent, double SphereRadius)
	{
		TSharedPtr<FJsonObject> BoundsObject = MakeShareable(new FJsonObject);
		BoundsObject->SetArrayField(TEXT("origin"), VectorToArray(Origin));
		BoundsObject->SetArrayField(TEXT("box_extent"), VectorToArray(BoxExtent));
		BoundsObject->SetArrayField(TEXT("box_min"), VectorToArray(Origin - BoxExtent));
		BoundsObject->SetArrayField(TEXT("box_max"), VectorToArray(Origin + BoxExtent));
		BoundsObject->SetNumberField(TEXT("sphere_radius"), SphereRadius);
		return BoundsObject;
	}

	bool TryGetActorBounds(AActor* Actor, FVector& OutOrigin, FVector& OutBoxExtent, double& OutSphereRadius)
	{
		if (!Actor)
		{
			return false;
		...
		Actor->GetActorBounds(false, OutOrigin, OutBoxExtent, false);
		OutSphereRadius = static_cast<double>(OutBoxExtent.Size());
		return true;
	}
...
namespace McpV2ToolUtils
{
	...
	{
		return TransformToJson(Transform);
	}
	...
	TSharedPtr<FJsonObject> BuildWorldSummary(UWorld* World, const FString& SessionId, bool bIncludeLevels, bool bIncludeSelection)
	{
		...
		{
			TSharedPtr<FJsonObject> SelectionSummary = BuildSelectionSummary(World, SessionId, false, false, TNumericLimits<int32>::Max());
			if (SelectionSummary.IsValid())
			{
				const TArray<TSharedPtr<FJsonValue>>* SelectedActors = nullptr;
				if (SelectionSummary->TryGetArrayField(TEXT("selected_actors"), SelectedActors) && SelectedActors)
				{
	...
	TSharedPtr<FJsonObject> BuildSelectionSummary(UWorld* World, const FString& SessionId, bool bIncludeTransform, bool bIncludeBounds, int32 Limit)
	{
		if (!World)
		{
			return nullptr;
		...
		if (GEditor && GEditor->GetSelectedActors())
		{
			for (FSelectionIterator It(*GEditor->GetSelectedActors()); It; ++It)
			{
				...
				if (SelectedActorsArray.Num() < Limit)
				{
					SelectedActorsArray.Add(MakeShareable(new FJsonValueObject(
						SerializeActorSummary(Actor, SessionId, bIncludeTransform, bIncludeBounds))));
				}
			}
		}

		Result->SetNumberField(TEXT("selection_count"), SelectionCount);
		Result->SetArrayField(TEXT("selected_actors"), SelectedActorsArray);
		return Result;
	}

	TSharedPtr<FJsonObject> SerializeBounds(const FVector& Origin, const FVector& BoxExtent, double SphereRadius)
	{
		return MakeBoundsJson(Origin, BoxExtent, SphereRadius);
	}

	TSharedPtr<FJsonObject> SerializeActorBounds(AActor* Actor)
	{
		FVector Origin = FVector::ZeroVector;
		FVector BoxExtent = FVector::ZeroVector;
		double SphereRadius = 0.0;
		if (!TryGetActorBounds(Actor, Origin, BoxExtent, SphereRadius))
		{
			return nullptr;
		}
	...
	TSharedPtr<FJsonObject> BuildAggregateBounds(const TArray<AActor*>& Actors)
	{
		bool bHasBounds = false;
		FBox AggregateBounds(EForceInit::ForceInit);
		...
		if (!bHasBounds)
		{
			return MakeBoundsJson(FVector::ZeroVector, FVector::ZeroVector, 0.0);
		}

		return MakeBoundsJson(
			AggregateBounds.GetCenter(),
			AggregateBounds.GetExtent(),
			static_cast<double>(AggregateBounds.GetExtent().Size()));
	}

	TSharedPtr<FJsonObject> SerializeActorSummary(AActor* Actor, const FString& SessionId, bool bIncludeTransform, bool bIncludeBounds)
	{
		if (!Actor)
		{
			return nullptr;
		...
		{
			ActorObject->SetObjectField(TEXT("transform"), SerializeTransform(Actor->GetActorTransform()));
		}
		if (bIncludeBounds)
		{
			if (TSharedPtr<FJsonObject> BoundsObject = SerializeActorBounds(Actor))
			{
	...
	TSharedPtr<FJsonObject> SerializeActorDetail(
		AActor* Actor,
		const FString& SessionId,
		bool bIncludeComponents,
		bool bIncludeProperties,
		bool bIncludeInherited,
		bool bIncludeHierarchy,
		bool bIncludeTags,
		bool bIncludeVisibility,
		bool bIncludeMobility,
		bool bIncludeBounds)
	{
		TSharedPtr<FJsonObject> Result = SerializeActorSummary(Actor, SessionId, true, false);
		if (!Result.IsValid())
		{
			return nullptr;
		}
		...
		{
			if (UPackage* LevelPackage = ActorLevel->GetPackage())
			{
				Result->SetStringField(TEXT("level_path"), LevelPackage->GetPathName());
			}
		}
		...
		if (bIncludeHierarchy)
		{
			...
			{
				FMcpEditorSessionManager::Get().RememberActor(SessionId, Actor->GetWorld()->GetPathName(), ParentActor);
				Result->SetObjectField(TEXT("parent_actor"), SerializeActorSummary(ParentActor, SessionId, false, false));
			}

			TArray<AActor*> ChildActors;
			Actor->GetAttachedActors(ChildActors);
			TArray<TSharedPtr<FJsonValue>> ChildActorsArray;
			for (AActor* ChildActor : ChildActors)
			{
				...
				FMcpEditorSessionManager::Get().RememberActor(SessionId, Actor->GetWorld()->GetPathName(), ChildActor);
				ChildActorsArray.Add(MakeShareable(new FJsonValueObject(SerializeActorSummary(ChildActor, SessionId, false, false))));
			}
			Result->SetArrayField(TEXT("child_actors"), ChildActorsArray);
		}
		...
		{
			Result->SetBoolField(TEXT("hidden_in_editor"), Actor->IsHiddenEd());
			Result->SetBoolField(TEXT("hidden_in_game"), Actor->IsHidden());
			Result->SetBoolField(TEXT("is_temporarily_hidden_in_editor"), Actor->IsTemporarilyHiddenInEditor());
		}

		if (bIncludeMobility)
		{
			const USceneComponent* RootSceneComponent = Actor->GetRootComponent();
			Result->SetStringField(TEXT("mobility"),
				RootSceneComponent
					? LevelActorToolUtils::MobilityToString(RootSceneComponent->Mobility)
					: TEXT("Unknown"));
		}
		...
		if (bIncludeBounds)
		{
			if (TSharedPtr<FJsonObject> BoundsObject = SerializeActorBounds(Actor))
			{
				Result->SetObjectField(TEXT("bounds"), BoundsObject);
			}
		}
