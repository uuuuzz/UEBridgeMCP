...
![QQ 群](https://img.shields.io/badge/QQ%20%E7%BE%A4-1102258979-12B7F5?logo=tencentqq&logoColor=white)

<details>
<summary>扫码加入 QQ 群</summary>

<img src="Docs/Images/qq-group-qrcode.jpg" alt="QQ 群二维码" width="300" />
...
- **动态工作流工具面** - 实时清单以 `tools/list` 为准，并会随条件工具、扩展模块和兼容别名变化
...
| [发布前预检](Docs/ReleasePreflight.zh-CN.md) | 发布口径下的运行时清单、兼容别名和安全探针检查 |
...
- 功能工具面与扩展模块还声明了 `ControlRig`、`PCG`、`Niagara`、`Metasound` 等可选依赖；这些模块是否可用会影响实时 `tools/list` 清单。
...
发布前可以在编辑器 MCP endpoint 在线后运行总预检：
...
插件描述文件当前声明了五个 Editor-only 模块：

- `UEBridgeMCP` - 共享协议类型、Schema 辅助、基类，以及编辑器侧 MCP 实现使用的工具 / resource / prompt 注册表
- `UEBridgeMCPEditor` - 编辑器侧服务、子系统集成、内置工具以及工具栏/状态集成
- `UEBridgeMCPControlRig` - Control Rig 扩展工具
- `UEBridgeMCPPCG` - PCG 扩展工具
- `UEBridgeMCPExternalAI` - 外部 AI 内容生成扩展工具
