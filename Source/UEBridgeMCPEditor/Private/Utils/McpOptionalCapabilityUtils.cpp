...
bool FMcpOptionalCapabilityUtils::IsNiagaraAvailable()
{
	return IsPluginEnabled(TEXT("Niagara")) &&
		HasModule(TEXT("Niagara")) &&
		HasModule(TEXT("NiagaraEditor"));
}

bool FMcpOptionalCapabilityUtils::IsMetaSoundAvailable()
{
	return IsPluginEnabled(TEXT("Metasound")) &&
		HasModule(TEXT("MetasoundEngine")) &&
		HasModule(TEXT("MetasoundFrontend")) &&
		HasModule(TEXT("MetasoundEditor"));
}
...
TSharedPtr<FJsonObject> FMcpOptionalCapabilityUtils::BuildOptionalCapabilities(UWorld* World)
{
	...
	Result->SetBoolField(TEXT("niagara_available"), IsNiagaraAvailable());
	Result->SetBoolField(TEXT("metasound_available"), IsMetaSoundAvailable());
