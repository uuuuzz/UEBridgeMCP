...
FString UCreateBlueprintFunctionTool::GetToolDescription() const
{
	return TEXT("Create a Blueprint function graph and optionally define its signature. Thin wrapper around edit-blueprint-members.");
}

TMap<FString, FMcpSchemaProperty> UCreateBlueprintFunctionTool::GetInputSchema() const
{
	TMap<FString, FMcpSchemaProperty> Schema;
	Schema.Add(TEXT("asset_path"), FMcpSchemaProperty::Make(TEXT("string"), TEXT("Blueprint asset path"), true));
	Schema.Add(TEXT("name"), FMcpSchemaProperty::Make(TEXT("string"), TEXT("Function name"), true));
...
TArray<FString> UCreateBlueprintFunctionTool::GetRequiredParams() const
{
	return {TEXT("asset_path"), TEXT("name")};
}

FMcpToolResult UCreateBlueprintFunctionTool::Execute(const TSharedPtr<FJsonObject>& Arguments, const FMcpToolContext& Context)
{
