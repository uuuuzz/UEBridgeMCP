...
namespace
{
	struct FExternalAIRequestState : public TSharedFromThis<FExternalAIRequestState, ESPMode::ThreadSafe>
	{
		FExternalAIRequestState()
		...
		TAtomic<bool> bRequestCompleted{ false };
		TAtomic<bool> bRequestSucceeded{ false };
	...
	FString& OutErrorMessage)
{
	...
	TSharedRef<FExternalAIRequestState, ESPMode::ThreadSafe> RequestState = MakeShared<FExternalAIRequestState, ESPMode::ThreadSafe>();

	HttpRequest->OnProcessRequestComplete().BindLambda(
		[RequestState](FHttpRequestPtr InRequest, FHttpResponsePtr InResponse, bool bSucceeded)
		{
			RequestState->HttpResponse = InResponse;
			RequestState->bRequestSucceeded.Store(bSucceeded);
			RequestState->bRequestCompleted.Store(true);
	...
	while (!RequestState->bRequestCompleted.Load() && FPlatformTime::Seconds() < TimeoutAt)
	{
		if (RequestState->CompletionEvent)
		{
			RequestState->CompletionEvent->Wait(100);
		}
	}

	if (!RequestState->bRequestCompleted.Load())
	{
	...
	if (!RequestState->bRequestSucceeded.Load() || !RequestState->HttpResponse.IsValid())
	{
		OutErrorCode = TEXT("UEBMCP_EXTERNAL_AI_REQUEST_FAILED");
	...
	OutResponse.HttpStatus = RequestState->HttpResponse->GetResponseCode();
	if (OutResponse.HttpStatus < 200 || OutResponse.HttpStatus >= 300)
	{
		OutErrorCode = TEXT("UEBMCP_EXTERNAL_AI_REQUEST_FAILED");
		OutErrorMessage = FString::Printf(TEXT("HTTP %d: %s"), OutResponse.HttpStatus, *RequestState->HttpResponse->GetContentAsString());
		return false;
	}

	TSharedPtr<FJsonObject> ResponseObject;
	TSharedRef<TJsonReader<>> Reader = TJsonReaderFactory<>::Create(RequestState->HttpResponse->GetContentAsString());
